	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			if (rs == null || !rs.next()) {
				...
				statement.executeUpdate("create table if not exists system (id INTEGER PRIMARY KEY AUTO_INCREMENT, display_nm varchar not null, user varchar not null, host varchar not null, port INTEGER not null, authorized_keys varchar not null, status_cd varchar not null default 'INITIAL', user_id INTEGER, foreign key (user_id) references users(id) on delete cascade)");
